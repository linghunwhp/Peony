...
def sensitive_filters(model_args, model, test_loader, mutual_classes, noise_var=0.5):
    sensitive_filters_all_file = 'results_shufflenetv2_tinyimagenet/sensitive_filters_all_' + str(noise_var) + '.txt'
    sensitive_filters_each_class_file = 'results_shufflenetv2_tinyimagenet/sensitive_filters_each_class_' + str(noise_var) + '.txt'
    ...
    if os.path.exists(sensitive_filters_all_file):
        ...
        state_dict_original = torch.load(model_args.weights)
        for layer in state_dict_original:
            ...
            if len(layer_size) == 4:
                ...
                for filter_index in range(layer_size[0]):
                    ...
                    state_dict_temp = torch.load(model_args.weights)
        ...
        print("filter_number_all is", filter_number_all, "filter_number_affected is ", filter_number_affected)
...
def controlled_training(model_args, model, train_loader, test_loader, mutual_classes, filters_for_aspects, noise_var=0.05):
    ...
    for mutual_class in mutual_classes:
        model.load_state_dict(torch.load(model_args.weights))
        mutual_mis_original.append({str(mutual_class[0])+'_to_'+str(mutual_class[1]): original_conf_matrix[mutual_class[0]][mutual_class[1]]*2,
                                    str(mutual_class[1])+'_to_'+str(mutual_class[0]): original_conf_matrix[mutual_class[1]][mutual_class[0]]*2})
        print("mutual_class: ", mutual_class, "original: train_acc: ", train_acc, "test_acc: ", test_acc, 
              str(mutual_class[0]), "_to_", str(mutual_class[0]), ":", original_conf_matrix[mutual_class[0]][mutual_class[0]]*2,
              str(mutual_class[1]), "_to_", str(mutual_class[1]), ":", original_conf_matrix[mutual_class[1]][mutual_class[1]]*2, 
              str(mutual_class[0]), "_to_", str(mutual_class[1]), ":", original_conf_matrix[mutual_class[0]][mutual_class[1]]*2, 
              str(mutual_class[1]), "_to_", str(mutual_class[0]), ":", original_conf_matrix[mutual_class[1]][mutual_class[0]]*2)
        ...
        for epoch in range(1, 26):
            for index, (images, labels) in enumerate(train_loader):
                ...
                for i, label in enumerate(labels):
                    ...
                    # else:
                    #     if labels_other is None:
                    #         images_other = images[i].unsqueeze(0)
                    #         labels_other = torch.tensor([label])
                    #     else:
                    #         images_other = torch.cat((images_other, images[i].unsqueeze(0)))
                    #         labels_other = torch.cat((labels_other, torch.tensor([label])))
                ...
                if model_args.gpu:
                    ...
                    # images_other = images_other.to(settings.CUDA)
                    # labels_other = labels_other.to(settings.CUDA)

                # 2. freeze all filters and unfreeze both sensitive filters, and then unlearn
                if images_mutual_class is not None:
                    # new_model_state_dict = freeze_filters(model_state_dict=model.state_dict(), filters_for_aspect=filters_for_aspect, reverse=True)
                ...
                for sub_epoch in range(1, 11):
                    new_model_state_dict = freeze_filters(model_state_dict=model.state_dict(), filters_for_aspect=filters_for_aspect, reverse=None)
            ...
            if conf_matrix[mutual_class[0]][mutual_class[0]] + conf_matrix[mutual_class[1]][mutual_class[1]] + 5 > \
                ...
                mutual_mis_all_epoch_temp.append({'mutual_class': mutual_class, 'epoch': epoch,
                                                  str(mutual_class[0])+'_to_'+str(mutual_class[0]): conf_matrix[mutual_class[0]][mutual_class[0]]*2,
                                                  str(mutual_class[1])+'_to_'+str(mutual_class[1]): conf_matrix[mutual_class[1]][mutual_class[1]]*2,
                                                  str(mutual_class[0])+'_to_'+str(mutual_class[1]): conf_matrix[mutual_class[0]][mutual_class[1]]*2,
                                                  str(mutual_class[1])+'_to_'+str(mutual_class[0]): conf_matrix[mutual_class[1]][mutual_class[0]]*2})
                torch.save(model.state_dict(), 'results_shufflenetv2_tinyimagenet/mutual_class_' + str(mutual_class) + '_noise_var_' + str(noise_var) + '_epoch_' + str(epoch) + '.pth')

            print("mutual_class: ", mutual_class, "epoch: ", epoch, "train_acc: ", train_acc, "test_acc: ", test_acc, 
                  str(mutual_class[0]), "_to_", str(mutual_class[0]), ":", conf_matrix[mutual_class[0]][mutual_class[0]]*2,
                  str(mutual_class[1]), "_to_", str(mutual_class[1]), ":", conf_matrix[mutual_class[1]][mutual_class[1]]*2, 
                  str(mutual_class[0]), "_to_", str(mutual_class[1]), ":", conf_matrix[mutual_class[0]][mutual_class[1]]*2, 
                  str(mutual_class[1]), "_to_", str(mutual_class[0]), ":", conf_matrix[mutual_class[1]][mutual_class[0]]*2)
        ...
        file = open('results_shufflenetv2_tinyimagenet/mutual_mis_all_epoch_var_' + str(noise_var) + '_unlearn_rate_' + str(unlearn_rate) + '_train_rate_' + str(train_rate) + '.txt', "w")
...
if __name__ == '__main__':
    ...
    mutual_misclassified_classes = [[73, 135], [30, 32], [70, 94]]

    # find all the sensitive filters corresponding to the classes
    filters_in_all_aspects, filters_in_3_aspects = sensitive_filters(model_args=args, model=net, test_loader=testing_loader, mutual_classes=mutual_misclassified_classes, noise_var=0.05)
    for filters_in_1_aspects in filters_in_3_aspects:
        print("Mutual_class: ", filters_in_1_aspects['mutual_class'], "Number of filters for both classes ", len(filters_in_1_aspects['sensitive_filters_class_both']),
              "Number of filters for class ", filters_in_1_aspects['mutual_class'][0], ":", len(filters_in_1_aspects['sensitive_filters_class_0']),
              "Number of filters for class ", filters_in_1_aspects['mutual_class'][1], ":", len(filters_in_1_aspects['sensitive_filters_class_1']))

    net.load_state_dict(torch.load(args.weights))
    if args.repairing:
        model_state_dicts_3_aspects = controlled_training(model_args=args, model=net, train_loader=training_loader, test_loader=testing_loader, mutual_classes=mutual_misclassified_classes, filters_for_aspects=filters_in_3_aspects, noise_var=0.05)
    else:
        print("Model Repair with Incremental Patching")
        state_dict_3 = []
        state_dict_3.append(torch.load('results_shufflenetv2_tinyimagenet/var_0.05/mutual_class_[73, 135]_noise_var_0.05_epoch_1.pth', map_location=settings.CUDA))
        state_dict_3.append(torch.load('results_shufflenetv2_tinyimagenet/var_0.05/mutual_class_[30, 32]_noise_var_0.05_epoch_2.pth', map_location=settings.CUDA))
        state_dict_3.append(torch.load('results_shufflenetv2_tinyimagenet/var_0.05/mutual_class_[70, 94]_noise_var_0.05_epoch_14.pth', map_location=settings.CUDA))
