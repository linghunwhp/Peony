...
def sensitive_filters(model_args, model, test_loader, mutual_classes, noise_var=0.5):
    sensitive_filters_all_file = 'results_mobilenet_tinyimagenet/sensitive_filters_all_' + str(noise_var) + '.txt'
    sensitive_filters_each_class_file = 'results_mobilenet_tinyimagenet/sensitive_filters_each_class_' + str(noise_var) + '.txt'
    ...
    if os.path.exists(sensitive_filters_all_file):
        ...
        for layer in state_dict_original:
            # print(param_tensor, '\t', state_dict_original[param_tensor].size())
    ...
    for mutual_class in mutual_classes:
        ...
        for epoch in range(1, 26):
            ...
            if conf_matrix[mutual_class[0]][mutual_class[0]] + conf_matrix[mutual_class[1]][mutual_class[1]] + 5 > \
                ...
                torch.save(model.state_dict(), 'results_mobilenet_tinyimagenet/mutual_class_' + str(mutual_class) + '_noise_var_' + str(noise_var) + '_epoch_' + str(epoch) + '.pth')
        ...
        file = open('results_mobilenet_tinyimagenet/mutual_mis_all_epoch_var_' + str(noise_var) + '_unlearn_rate_' + str(unlearn_rate) + '_train_rate_' + str(train_rate) + '.txt', "w")
...
if __name__ == '__main__':
    ...
    mutual_misclassified_classes = [[30, 32], [73, 135], [70, 94]]
    ...
    if args.repairing:
        ...
        state_dict_3 = []
        state_dict_3.append(torch.load('results_mobilenet_tinyimagenet/var_0.05/mutual_class_[30, 32]_noise_var_0.05_epoch_3.pth', map_location=settings.CUDA))
        state_dict_3.append(torch.load('results_mobilenet_tinyimagenet/var_0.05/mutual_class_[73, 135]_noise_var_0.05_epoch_12.pth', map_location=settings.CUDA))
        state_dict_3.append(torch.load('results_mobilenet_tinyimagenet/var_0.05/mutual_class_[70, 94]_noise_var_0.1_epoch_12.pth', map_location=settings.CUDA))
